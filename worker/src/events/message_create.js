...
module.exports = class extends Event {
  ...
  async exec(msg) {
    ...
    const cmd = extractCommand(
      (s) => (s.startsWith(prefix) ? prefix.length : null),
      tokens
    );
    if (!cmd) return;

    const command =
      this.client.handler.commands.get(cmd.value.toLowerCase()) ||
      this.client.handler.commands.get(
        this.client.handler.aliases.get(cmd.value.toLowerCase())
      );
    if (!command) return;
    ...
    try {
      ...
      Logger.get('message_create').debug(
        `${msg.author.id} ran ${command.name}`
      );
    } catch (e) {
